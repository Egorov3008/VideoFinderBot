        ...
        if user := data.get("event_from_user"):
            ...
            if dict_sub:
                ...
                if not any(v for v in dict_sub_users.values() if v):
                    ...
                    if event.message:
                        await event.message.answer(f"😊 Чтобы использовать функции бота, подпишитесь на каналы,\n перечисленные ниже"
                    ...
                    if event.callback_query:
                        await event.callback_query.message.answer(f"😊 Чтобы использовать функции бота, подпишитесь на каналы,\n перечисленные ниже"
                                                   f"и нажмите кнопку 'Проверить подписку 📍',\n как только вы подпишетесь.", reply_markup=builder.as_markup())
